...
**Current Status**: Functional with extended diagram features (areas, notes, custom types, dependencies, filters)
**Go Version**: 1.25.6
...
| `GET` | `/diagrams/:id` | Get single diagram with full content (tables, relationships, areas, notes, custom types, dependencies, filters) |
...
│   ├── mongo_area_repository.go
│   ├── mongo_note_repository.go
│   ├── mongo_custom_type_repository.go
│   ├── mongo_dependency_repository.go
│   ├── mongo_diagram_filter_repository.go
...
## 🆕 Extended Features (Recent Additions)

### Areas
Grouping visual containers to organize tables within diagrams
- Spatial positioning (x, y, width, height)
- Color coding for visual distinction
- Parent container for logical grouping

### Notes
Text annotations and documentation within diagrams
- Free-form text content
- Spatial positioning
- Color customization
- For diagram documentation and comments

### Custom Types
Domain-specific type definitions beyond standard SQL types
- Store custom type definitions
- Use in table field definitions
- Support for domain-driven design

### Dependencies
Explicit dependency relationships between tables (beyond foreign keys)
- Different from relationships (captures logical dependencies)
- Useful for ETL pipelines and data lineage
- Type classification for dependency categorization

### Diagram Filters
Filter and view configurations for diagrams
- Save filter rules for diagram views
- Conditional display of tables based on criteria
- Support for multiple filter sets per diagram
